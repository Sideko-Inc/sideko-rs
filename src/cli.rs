...
async fn handle_cli(cli: SidekoCli) -> sideko_rest_api::SdkResult<()> {
    ...
    match cli.command {
        ...
        SidekoCommand::DeleteApiProject(req) => {
            let mut client = sideko_rest_api::Client::default();
            if let Ok(base_url) = std::env::var(base_url_env_var) {
                client = client.with_base_url(&base_url);
                log::debug!("Using custom base url: {base_url}");
            ...
            client.delete_api_project(req).await?;
            log::info!("API returned no content");
        }
        SidekoCommand::DeleteApiProjectRole(req) => {
            let mut client = sideko_rest_api::Client::default();
            if let Ok(base_url) = std::env::var(base_url_env_var) {
                client = client.with_base_url(&base_url);
                log::debug!("Using custom base url: {base_url}");
            ...
            client.delete_api_project_role(req).await?;
            log::info!("API returned no content");
        }
        ...
        SidekoCommand::GetApiProject(req) => {
            let mut client = sideko_rest_api::Client::default();
            if let Ok(base_url) = std::env::var(base_url_env_var) {
                client = client.with_base_url(&base_url);
                log::debug!("Using custom base url: {base_url}");
            ...
            let res = client.get_api_project(req).await?;
            println!(
                "{}", serde_json::to_string_pretty(& res).unwrap_or_else(| _ |
                serde_json::json!(& res) .to_string())
            );
        }
        SidekoCommand::ListApiProjectMembers(req) => {
            let mut client = sideko_rest_api::Client::default();
            if let Ok(base_url) = std::env::var(base_url_env_var) {
                client = client.with_base_url(&base_url);
                log::debug!("Using custom base url: {base_url}");
            ...
            let res = client.list_api_project_members(req).await?;
            println!(
                "{}", serde_json::to_string_pretty(& res).unwrap_or_else(| _ |
                serde_json::json!(& res) .to_string())
            );
        }
        SidekoCommand::ListApiVersions(req) => {
            let mut client = sideko_rest_api::Client::default();
            if let Ok(base_url) = std::env::var(base_url_env_var) {
                client = client.with_base_url(&base_url);
                log::debug!("Using custom base url: {base_url}");
            ...
            let res = client.list_api_versions(req).await?;
            println!(
                "{}", serde_json::to_string_pretty(& res).unwrap_or_else(| _ |
                serde_json::json!(& res) .to_string())
            );
        }
        SidekoCommand::GetApiVersion(req) => {
            let mut client = sideko_rest_api::Client::default();
            if let Ok(base_url) = std::env::var(base_url_env_var) {
                client = client.with_base_url(&base_url);
                log::debug!("Using custom base url: {base_url}");
            ...
            let res = client.get_api_version(req).await?;
            println!(
                "{}", serde_json::to_string_pretty(& res).unwrap_or_else(| _ |
                serde_json::json!(& res) .to_string())
            );
        }
        SidekoCommand::GetApiVersionOpenapi(req) => {
            let mut client = sideko_rest_api::Client::default();
            if let Ok(base_url) = std::env::var(base_url_env_var) {
                client = client.with_base_url(&base_url);
                log::debug!("Using custom base url: {base_url}");
            ...
            let res = client.get_api_version_openapi(req).await?;
            println!(
                "{}", serde_json::to_string_pretty(& res).unwrap_or_else(| _ |
                serde_json::json!(& res) .to_string())
            );
        ...
        SidekoCommand::UpdateApiProject(req) => {
            let mut client = sideko_rest_api::Client::default();
            if let Ok(base_url) = std::env::var(base_url_env_var) {
                client = client.with_base_url(&base_url);
                log::debug!("Using custom base url: {base_url}");
            ...
            let res = client.update_api_project(req).await?;
            println!(
                "{}", serde_json::to_string_pretty(& res).unwrap_or_else(| _ |
                serde_json::json!(& res) .to_string())
            );
        }
        SidekoCommand::UpdateApiVersion(req) => {
            let mut client = sideko_rest_api::Client::default();
            if let Ok(base_url) = std::env::var(base_url_env_var) {
                client = client.with_base_url(&base_url);
                log::debug!("Using custom base url: {base_url}");
            ...
            let res = client.update_api_version(req).await?;
            println!(
                "{}", serde_json::to_string_pretty(& res).unwrap_or_else(| _ |
                serde_json::json!(& res) .to_string())
            );
        ...
        SidekoCommand::CreateApiProject(req) => {
            let mut client = sideko_rest_api::Client::default();
            if let Ok(base_url) = std::env::var(base_url_env_var) {
                client = client.with_base_url(&base_url);
                log::debug!("Using custom base url: {base_url}");
            ...
            let res = client.create_api_project(req).await?;
            println!(
                "{}", serde_json::to_string_pretty(& res).unwrap_or_else(| _ |
                serde_json::json!(& res) .to_string())
            );
        }
        SidekoCommand::GrantApiProjectRole(req) => {
            let mut client = sideko_rest_api::Client::default();
            if let Ok(base_url) = std::env::var(base_url_env_var) {
                client = client.with_base_url(&base_url);
                log::debug!("Using custom base url: {base_url}");
            ...
            client.grant_api_project_role(req).await?;
            log::info!("API returned no content");
        }
        SidekoCommand::CreateApiVersion(req) => {
            let mut client = sideko_rest_api::Client::default();
            if let Ok(base_url) = std::env::var(base_url_env_var) {
                client = client.with_base_url(&base_url);
                log::debug!("Using custom base url: {base_url}");
            ...
            let res = client.create_api_version(req).await?;
            println!(
                "{}", serde_json::to_string_pretty(& res).unwrap_or_else(| _ |
                serde_json::json!(& res) .to_string())
            );
...
enum SidekoCommand {
    ...
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api delete-api-project --project-id-or-name string`
    #[command(name = "delete-api-project")]
    DeleteApiProject(sideko_rest_api::DeleteApiProjectRequest),
    /// command
    ///
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api delete-api-project-role --project-id-or-name string --user-id string`
    #[command(name = "delete-api-project-role")]
    DeleteApiProjectRole(sideko_rest_api::DeleteApiProjectRoleRequest),
    /// command
    ///
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    ...
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api list-api-projects`
    #[command(name = "list-api-projects")]
    ListApiProjects,
    /// command
    ///
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api get-api-project --project-id-or-name string`
    #[command(name = "get-api-project")]
    GetApiProject(sideko_rest_api::GetApiProjectRequest),
    /// command
    ///
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api list-api-project-members --project-id-or-name string`
    #[command(name = "list-api-project-members")]
    ListApiProjectMembers(sideko_rest_api::ListApiProjectMembersRequest),
    /// command
    ///
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api list-api-versions --project-id-or-name string`
    #[command(name = "list-api-versions")]
    ListApiVersions(sideko_rest_api::ListApiVersionsRequest),
    /// command
    ///
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api get-api-version --project-id-or-name string --version-id-or-semver string`
    #[command(name = "get-api-version")]
    GetApiVersion(sideko_rest_api::GetApiVersionRequest),
    /// command
    ///
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api get-api-version-openapi --project-id-or-name string --version-id-or-semver string`
    #[command(name = "get-api-version-openapi")]
    GetApiVersionOpenapi(sideko_rest_api::GetApiVersionOpenapiRequest),
    /// command
    ///
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api get-api-version-stats --project-id-or-name string --version-id-or-semver string`
    ...
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api list-sdks --api-id-or-name string`
    ...
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api update-api-link --link-id 3e4666bf-d5e5-4aa7-b8ce-cefe41c7568a --api-version-id 3e4666bf-d5e5-4aa7-b8ce-cefe41c7568a --build-request-enabled true --include-mock-server true --nav-label string --policy latest --slug string`
    ...
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api update-api-project --project-id-or-name string --title my-new-api-name`
    #[command(name = "update-api-project")]
    UpdateApiProject(sideko_rest_api::UpdateApiProjectRequest),
    /// command
    ///
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api update-api-version --project-id-or-name string --version-id-or-semver string --mock-server-enabled true --notes '<p>This version includes a number of excellent improvements</p>' --openapi string --semver string`
    #[command(name = "update-api-version")]
    UpdateApiVersion(sideko_rest_api::UpdateApiVersionRequest),
    /// command
    ///
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    ...
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api create-api-link --api-version-id 3e4666bf-d5e5-4aa7-b8ce-cefe41c7568a --build-request-enabled true --doc-version-id 3e4666bf-d5e5-4aa7-b8ce-cefe41c7568a --group-id 3e4666bf-d5e5-4aa7-b8ce-cefe41c7568a --include-mock-server true --nav-label string --policy '{"api_project_id":"3e4666bf-d5e5-4aa7-b8ce-cefe41c7568a","type":"latest"}' --slug string`
    ...
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api create-api-project --title my-api-spec`
    #[command(name = "create-api-project")]
    CreateApiProject(sideko_rest_api::CreateApiProjectRequest),
    /// command
    ///
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api grant-api-project-role --project-id-or-name string --role admin --user-id 3e4666bf-d5e5-4aa7-b8ce-cefe41c7568a`
    #[command(name = "grant-api-project-role")]
    GrantApiProjectRole(sideko_rest_api::GrantApiProjectRoleRequest),
    /// command
    ///
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api create-api-version --project-id-or-name string --mock-server-enabled true --notes '<p>This version includes a number of excellent improvements</p>' --openapi string --semver string`
    #[command(name = "create-api-version")]
    CreateApiVersion(sideko_rest_api::CreateApiVersionRequest),
    /// command
    ///
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    ...
    /// **Required Auth:** CookieAuth OR ApiKeyAuth
    ///
    /// **Example:** `sideko-rest-api update-sdk --name my-python-sdk --semver 0.1.1 --api-version-id-or-semver string --file ./tests/file.pdf`
...
mod cli_tests {
    ...
    #[serial_test::serial]
    #[tokio::test]
    async fn test_cli_delete_api_project_204_generated_success() {
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(
                    &[
                        "sideko-rest-api",
    ...
    #[serial_test::serial]
    #[tokio::test]
    async fn test_cli_delete_api_project_role_204_generated_success() {
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(
                    &[
                        "sideko-rest-api",
    ...
    #[serial_test::serial]
    #[tokio::test]
    async fn test_cli_list_api_projects_200_generated_success() {
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(
                    &[
                        "sideko-rest-api",
        ...
        super::handle_cli(cli).await.expect("failed running auth command");
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(&["sideko-rest-api", "list-api-projects"].join(" ")),
            )
            .expect("failed parsing cli input");
        let result = super::handle_cli(cli).await;
        println!("{:?}", & result);
    ...
    #[serial_test::serial]
    #[tokio::test]
    async fn test_cli_get_api_project_200_generated_success() {
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(
                    &[
                        "sideko-rest-api",
    ...
    #[serial_test::serial]
    #[tokio::test]
    async fn test_cli_list_api_project_members_200_generated_success() {
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(
                    &[
                        "sideko-rest-api",
    ...
    #[serial_test::serial]
    #[tokio::test]
    async fn test_cli_list_api_versions_200_generated_success() {
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(
                    &[
                        "sideko-rest-api",
    ...
    #[serial_test::serial]
    #[tokio::test]
    async fn test_cli_get_api_version_200_generated_success() {
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(
                    &[
                        "sideko-rest-api",
    ...
    #[serial_test::serial]
    #[tokio::test]
    async fn test_cli_get_api_version_openapi_200_generated_success() {
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(
                    &[
                        "sideko-rest-api",
    ...
    async fn test_cli_list_sdks_200_generated_success() {
        ...
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(
                    &["sideko-rest-api", "list-sdks", "--api-id-or-name", "string"]
                        .join(" "),
                ),
            )
            .expect("failed parsing cli input");
    ...
    #[serial_test::serial]
    #[tokio::test]
    async fn test_cli_update_api_project_200_success_default() {
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(
                    &[
                        "sideko-rest-api",
    ...
    #[serial_test::serial]
    #[tokio::test]
    async fn test_cli_update_api_version_200_success_default() {
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(
                    &[
                        "sideko-rest-api",
        ...
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(
                    &[
                        ...
                        "string",
                        "--policy",
                        "'{\"api_project_id\":\"3e4666bf-d5e5-4aa7-b8ce-cefe41c7568a\",\"type\":\"latest\"}'",
                        "--slug",
                        "string",
                    ]
                        .join(" "),
    ...
    #[serial_test::serial]
    #[tokio::test]
    async fn test_cli_create_api_project_201_success_default() {
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(
                    &[
                        "sideko-rest-api",
        ...
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(
                    &["sideko-rest-api", "create-api-project", "--title", "my-api-spec"]
                        .join(" "),
                ),
            )
            .expect("failed parsing cli input");
    ...
    #[serial_test::serial]
    #[tokio::test]
    async fn test_cli_grant_api_project_role_201_success_default() {
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(
                    &[
                        "sideko-rest-api",
    ...
    #[serial_test::serial]
    #[tokio::test]
    async fn test_cli_create_api_version_201_success_default() {
        let cli = super::SidekoCli::try_parse_from(
                shlex::Shlex::new(
                    &[
                        "sideko-rest-api",
